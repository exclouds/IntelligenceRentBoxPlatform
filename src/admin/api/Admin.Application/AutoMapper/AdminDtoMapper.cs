...
namespace Magicodes.Admin.AutoMapper
{
    internal static class AdminDtoMapper
    {
        public static void CreateMappings(IMapperConfigurationExpression configuration)
        {
            ...
            configuration.CreateMap<User, UserListExtionDto>()
                .ForMember(dto => dto.TelNumber, options => options.MapFrom(p => p.SignInToken))
                .ForMember(dto => dto.Sex, options => options.MapFrom(p => p.Sex));

            configuration.CreateMap<User, NewUserEditDto>()
                .ForMember(dto => dto.TelNumber, options => options.MapFrom(p => p.SignInToken))
                .ForMember(dto => dto.Sex, options => options.MapFrom(p => p.Sex))
                .ForMember(dto => dto.IsActive, options => options.MapFrom(p => p.IsLockoutEnabled));
