...
namespace Abp.Castle.Logging.NLog
{
		...
		ILogger
	{
		...
		{
			//字符串拼接格式
			//接口名$原始数据$修改后数据$调用json$操作人$操作时间
			//string[] msgArr = message.Trim('$').Split('$');

			//LogEventInfo ei = new LogEventInfo(LogLevel.Error, Logger.Name, message);
			//if (msgArr.Length > 0)
			//{
			//    ei.Properties["InterfaceName"] = msgArr[0];
			//}
			//else if (msgArr.Length > 1)
			//{
			//    ei.Properties["OriginalData"] = msgArr[1];
			//}
			//else if (msgArr.Length > 2)
			//{
			//    ei.Properties["SaveData"] = msgArr[2];
			//}
			//else if (msgArr.Length > 3)
			//{
			//    ei.Properties["InputJson"] = msgArr[3];
			//}
			//else if (msgArr.Length > 4)
			//{
			//    ei.Properties["User"] = msgArr[4];
			//}
			//else if (msgArr.Length > 5)
			//{
			//    ei.Properties["ServiceName"] = ServiceName;
			//}
